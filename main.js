...
function extractCompletionEndTime(text) {
  ...
  if (!match) return null;
  return (_a = match[2]) != null ? _a : match[1];
}
function findLatestCompletionEndTime(lines, referenceTime) {
  ...
  let latest = null;
  let latestBeforeReference = null;
  ...
  return (_b = (_a = latestBeforeReference == null ? void 0 : latestBeforeReference.time) != null ? _a : latest == null ? void 0 : latest.time) != null ? _b : null;
}
...
var TaskParser = class {
  ...
  static parseLine(line) {
    ...
    const matchedTimes = (_a = remaining.match(timePattern)) != null ? _a : [];
    const times = leadingTimeBlock ? [...leadingTimeBlock.times, ...matchedTimes] : matchedTimes;
    if (!leadingTimeBlock) {
  ...
  static extractLeadingTimeBlock(remaining) {
...
var TASK_STATUS_PREFIX_REGEX = /^- \[( |\/|x)\] ?/;
function transformTaskLine(lineText, now, forceAction) {
  ...
  if (timeCount === 1) {
    if (lineText.startsWith("- [ ]")) {
      let content2 = lineText.replace(/^- \[ \] ?/, "").trim();
      content2 = content2.replace(/^\d{2}:\d{2}\s*-?\s*/, "").trim();
      return {
        type: "update",
        content: `- [/] ${timeStr} - ${content2}`
      };
    }
...
function transformCheckboxPress(lineText, now, intent, options = {}) {
  ...
  if (isUnstarted) {
    return buildStartResult(lineText, (_a = options.unstartedLongPressStartTime) != null ? _a : timeStr, {
...
function adjustTaskTimeByMinutes(lineText, deltaMinutes) {
  ...
  const startIndex = (_a = target.index) != null ? _a : -1;
...
function normalizeCompletedTaskActualDuration(lineText) {
  ...
  const times = (_a = lineText.match(/\d{2}:\d{2}/g)) != null ? _a : [];
...
function cloneResult(lineText, timeStr, options) {
  const doneEstimateMinutes = extractDoneEstimateMinutes(lineText);
  const doneActualMinutes = extractDoneActualMinutes(lineText);
  const nextEstimateMinutes = doneEstimateMinutes !== null && doneActualMinutes !== null ? Math.max(doneEstimateMinutes - doneActualMinutes, 0) : null;
  let taskPart = lineText.replace(TASK_STATUS_PREFIX_REGEX, "");
...
function extractDoneEstimateMinutes(lineText) {
...
function extractDoneActualMinutes(lineText) {
  var _a;
  const times = (_a = lineText.match(/\d{2}:\d{2}/g)) != null ? _a : [];
  if (times.length >= 2) {
...
function buildStartResult(lineText, startTime, options = {}) {
  var _a;
  const resolved = resolveStartableTask(lineText, startTime, {
    preferPlannedStart: (_a = options.preferPlannedStart) != null ? _a : true
  });
  return { type: "update", content: `- [/] ${resolved.startTime} - ${resolved.content}` };
}
function buildResetToUnstartedResult(lineText) {
  var _a, _b;
  const isRunning = lineText.startsWith("- [/]");
  const isComplete = lineText.startsWith("- [x]");
  if (!isRunning && !isComplete) return null;
  const body = lineText.replace(TASK_STATUS_PREFIX_REGEX, "").trim();
  const runningStartTime = isRunning ? (_b = (_a = body.match(/^(\d{2}:\d{2})\s*-\s*/)) == null ? void 0 : _a[1]) != null ? _b : null : null;
  const estimateMinutes = extractEstimateMinutesForReset(lineText);
  let taskPart = body;
  if (isRunning) {
    taskPart = taskPart.replace(/^\d{2}:\d{2}\s*-\s*/, "").trim();
  } else {
    taskPart = taskPart.replace(/^\d{2}:\d{2}\s*-\s*\d{2}:\d{2}\s*/, "").replace(/^\d{2}:\d{2}\s*/, "").trim();
  }
...
function extractEstimateMinutesForReset(lineText) {
  const estimateVsActual = lineText.match(/\((\d+)m\s*>\s*\d+m\)/);
  if (estimateVsActual) return Number(estimateVsActual[1]);
  const singleDuration = lineText.match(/\((\d+)m\)/);
  if (singleDuration) return Number(singleDuration[1]);
...
function normalizeLooseUnstartedBody(body) {
...
function resolveStartableTask(lineText, fallbackStartTime, options = {}) {
  var _a, _b, _c;
  let content = lineText.replace(TASK_STATUS_PREFIX_REGEX, "").trim();
  ...
  const shouldPreferPlannedStart = (_a = options.preferPlannedStart) != null ? _a : true;
  const plannedStartTime = shouldPreferPlannedStart ? (_c = (_b = content.match(/^(\d{2}:\d{2})\s*-\s*/)) == null ? void 0 : _b[1]) != null ? _c : fallbackStartTime : fallbackStartTime;
...
function formatEstimateToken(rawValue, rawUnit) {
...
function extractTaskNameForReset(lineText) {
...
function buildInterruptResult(lineText, now) {
  ...
  const timeInfo = estimateMin > 0 && estimateMin !== actualDuration ? `(${estimateMin}m > ${actualDuration}m)` : `(${actualDuration}m)`;
...
function stripInterruptMarker(text) {
...
function parseMonthlyOn(text) {
  ...
  if (nthList.length > 1 && nthEntries.every(Boolean)) {
    const validEntries = nthEntries.filter((m) => m !== null);
...
var RoutineEngine = class {
  ...
  resolveFrequency(rawRepeat, rawFrequency, rawSchedule) {
    ...
    if (rawFrequency == null) return void 0;
  ...
  isRolloverEnabled(note) {
    ...
    return (_b = note.rollover) != null ? _b : true;
  }
  ...
  async normalizeOverdueNextDue(note, targetDate) {
   ...
   */
  async processCompletion(routineNote, completionDate) {
    ...
    try {
      let repeatToAppend = void 0;
      if (!frequency) {
        frequency = { type: "schedule", expression: "every day" };
        repeatToAppend = 1;
        this.emitNotice("LLR: \u6BCE\u65E5\u30EA\u30D4\u30FC\u30C8\u3092\u8A2D\u5B9A\u3057\u307E\u3057\u305F", 3e3);
      }
      const completionDay = this.normalizeToDateOnly(completionDate);
      const isDueAnchored = usesDueAnchor(frequency);
      const newNextDue = isDueAnchored ? this.calculateNextDueForDueAnchor(frequency, next_due, completionDay) : calculateNextDue(frequency, completionDay);
   ...
   */
  scheduleUpdate(routineFile, sourcePath, completionDate) {
    ...
    for (const [key, timer] of this.pendingTimers.entries()) {
      ...
      if (file instanceof import_obsidian.TFile) {
        ...
        if (routineNote) {
          ...
          await this.processCompletion(routineNote, /* @__PURE__ */ new Date());
        }
      }
    }
  ...
  async fetchDueRoutines(today) {
    ...
    for (const child of folder.children) {
      ...
      if (!note || !note.frequency) continue;
...
var _SummaryView = class _SummaryView extends import_obsidian2.ItemView {
  ...
  getRoutineSectionDefinitions() {
    var _a, _b, _c, _d, _e;
    const raw = (_e = (_d = (_c = (_b = (_a = this.app.plugins) == null ? void 0 : _a.plugins) == null ? void 0 : _b.llr) == null ? void 0 : _c.settings) == null ? void 0 : _d.sectionDefinitions) != null ? _e : [];
    if (!Array.isArray(raw)) return [];
    return raw.map((x) => {
      var _a2, _b2;
      const time = String((_a2 = x == null ? void 0 : x.time) != null ? _a2 : "");
      const label = String((_b2 = x == null ? void 0 : x.label) != null ? _b2 : "").trim();
...
function normalizeSectionDefinitions(input) {
  ...
  for (const item of input) {
    ...
    const rawTime = String((_a = item.time) != null ? _a : "").replace(/[^\d]/g, "").slice(0, 4);
    const label = String((_b = item.label) != null ? _b : "").trim();
    if (!label) continue;
  ...
  normalized.sort((a, b) => {
    var _a2, _b2;
    const av = (_a2 = parseSectionTimeToInt(a.time)) != null ? _a2 : Number.MAX_SAFE_INTEGER;
    const bv = (_b2 = parseSectionTimeToInt(b.time)) != null ? _b2 : Number.MAX_SAFE_INTEGER;
...
var LlrPlugin = class extends import_obsidian3.Plugin {
  ...
  async onload() {
    ...
    this.registerDomEvent(document, "input", (ev) => this.handleDocumentInput(ev), true);
  ...
  migrateLegacySkipCommandHotkeys() {
    ...
    if ((oldHasKeys || (customKeys == null ? void 0 : customKeys[oldCommandId]) != null) && typeof hotkeyManager.save === "function") {
      hotkeyManager.save();
...
`);
  ...
  async activateView() {
    ...
    if (leaf) {
      ...
      if (import_obsidian3.Platform.isMobile) {
        ...
        (_b = (_a = ws.leftSplit) == null ? void 0 : _a.collapse) == null ? void 0 : _b.call(_a);
        (_d = (_c = ws.rightSplit) == null ? void 0 : _c.expand) == null ? void 0 : _d.call(_c);
      }
    }
  }
  ...
  getDailyNoteSettings() {
    var _a, _b, _c, _d;
    const dailyNotesPlugin = (_b = (_a = this.app.internalPlugins) == null ? void 0 : _a.getPluginById) == null ? void 0 : _b.call(_a, "daily-notes");
    const options = (_d = (_c = dailyNotesPlugin == null ? void 0 : dailyNotesPlugin.instance) == null ? void 0 : _c.options) != null ? _d : {};
  ...
  buildRoutineCompletionSnapshot(file) {
    ...
    for (const item of listItems) {
      ...
      for (; scanIndex < links.length; scanIndex++) {
        ...
        const prevCounts = (_b = currentSnapshot.get(routineFile.path)) != null ? _b : { totalCount: 0, completedCount: 0 };
  ...
  scheduleDailyNoteRoutineAutoInsert(file, attempt = 0) {
    if (!this.isDailyNoteFile(file)) return;
    if (attempt > 12) {
  ...
  async tryAutoInsertRoutinesFromTemplateMarker(file, attempt) {
    var _a;
    try {
      if (!this.isDailyNoteFile(file)) return;
      const content = await this.app.vault.read(file);
  ...
  getCM6View(editor) {
    ...
    const raw = editor;
    const cm = raw.cm;
    const editorChain = raw.editor;
    const editorCm = editorChain == null ? void 0 : editorChain.cm;
    return (_f = (_e = (_d = (_b = (_a = cm == null ? void 0 : cm.cm) != null ? _a : cm) != null ? _b : raw.cmEditor) != null ? _d : (_c = editorCm == null ? void 0 : editorCm.cm) == null ? void 0 : _c.view) != null ? _e : editorCm) != null ? _f : null;
  }
  ...
  onMetadataChanged(file) {
    ...
    for (const routinePath of routinePaths) {
      const current = (_a = currentSnapshot.get(routinePath)) != null ? _a : { totalCount: 0, completedCount: 0 };
      const prev = (_b = previousSnapshot.get(routinePath)) != null ? _b : { totalCount: 0, completedCount: 0 };
      ...
      const completionBaseDate = resolveReferenceDate(this.parseDailyNoteDate(file), /* @__PURE__ */ new Date());
  ...
  handleFixDurationDriftAll(editor, view) {
  ...
  handleDeferTaskToTomorrow(editor, view) {
  ...
  async applyTaskResult(editor, view, lineIndex, lineText, result) {
    switch (result.type) {
      case "update":
...
${result.extraContent}`, { line: lineIndex, ch: result.content.length });
  ...
  async completeTask(editor, view, lineIndex, lineText) {
    const now = /* @__PURE__ */ new Date();
...
var LlrSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName(this.plugin.t("settings.routineSections.heading")).setDesc(this.plugin.t("settings.routineSections.desc")).setHeading();
  ...
  renderSectionDefinitionSettings(containerEl) {
    ...
    defs.forEach((def, index) => {
      ...
      }).addExtraButton((btn) => btn.setIcon("trash").setTooltip(this.plugin.t("settings.routineSections.deleteTooltip")).onClick(async () => {
        defs.splice(index, 1);
        await this.plugin.setSectionDefinitions(defs);
        this.display();
      }));
  ...
  renderAdvancedSettings(containerEl) {
    new import_obsidian3.Setting(containerEl).setName(this.plugin.t("settings.advanced.heading")).setDesc(this.plugin.t("settings.advanced.desc")).setHeading();
